  ...
  async activate(context) {
    ...
    const openMarketplace = () => {
      ...
      context.workbench.openTab({
        id: `extensions.marketplace.details.${EXTENSION_ID}`,
        kind: 'extensions.marketplace.details',
        surfaceId: 'extensions.marketplace.details.surface',
        title: 'API Studio',
        ownerExtensionId: 'activelane.extensions-marketplace',
        input: { surface: 'details', extensionId: EXTENSION_ID },
        preview: false,
      })
    }
    const selectedRequest = () =>
      studio.selectedRequestId ??
      (context.workbench.getActiveTab()?.input?.requestId as string | undefined)
    ...
    const contributions = [
      ...
      context.contribute.tabRenderers({
        id: 'api-studio.request-editor',
        title: 'API Studio Request Editor',
        tabKind: 'api-studio.request',
        component: ApiStudioRequestEditor,
      }),
      context.contribute.inspectorPanels({
        id: 'api-studio.request-inspector',
        title: 'Request Inspector',
        component: ApiStudioInspector,
        tabKinds: ['api-studio.request'],
      }),
      ...
      context.contribute.statusBar(
        ...
        {
          id: 'api-studio.plan',
          title: 'API Studio plan',
          label: context.entitlements.plan() === 'free' ? 'Free' : 'Pro ●',
          commandId: 'api-studio.open-marketplace',
          alignment: 'right',
          order: 40,
        },
      ),
      context.contribute.commands(
        {
          id: 'api-studio.open',
          title: 'API Studio: Open',
          category: 'API Studio',
          run: openStudio,
        },
        {
          id: 'api-studio.new-request',
          title: 'API Studio: New Request',
          category: 'API Studio',
          shortcut: 'Mod+Alt+N',
          run: async () => {
            openStudio()
            await studio.newRequest()
          },
        },
        {
          id: 'api-studio.new-collection',
          title: 'API Studio: New Collection',
          category: 'API Studio',
          run: async () => {
            openStudio()
            await studio.newCollection()
          },
        },
        {
          id: 'api-studio.send-request',
          title: 'API Studio: Send Request',
          category: 'API Studio',
          shortcut: 'Mod+Enter',
          run: async () => {
            const id = selectedRequest()
            if (id) await studio.send(id)
          },
        },
        {
          id: 'api-studio.save-request',
          title: 'API Studio: Save Request',
          category: 'API Studio',
          shortcut: 'Mod+S',
          run: () => studio.save(),
        },
        {
          id: 'api-studio.duplicate-request',
          title: 'API Studio: Duplicate Request',
          category: 'API Studio',
          run: async () => {
            const id = selectedRequest()
            if (id) await studio.duplicateRequest(id)
          },
        },
        {
          id: 'api-studio.select-environment',
          title: 'API Studio: Select Environment',
          category: 'API Studio',
          run: async () => {
            const environments = studio.state.environments
            const current = Math.max(
              0,
              environments.findIndex((item) => item.id === studio.state.activeEnvironmentId),
            )
        ...
        {
          id: 'api-studio.clear-history',
          title: 'API Studio: Clear Request History',
          category: 'API Studio',
          run: () => studio.clearHistory(),
        },
        {
          id: 'api-studio.open-marketplace',
          title: 'API Studio: Open Marketplace Details',
          category: 'API Studio',
          run: openMarketplace,
        },
        {
          id: 'api-studio.toggle-log',
          title: 'API Studio: Toggle Request Log',
          category: 'API Studio',
          run: () => {
            context.workbench.setActiveBottomPanelView('api-studio.request-log')
      ...
      context.contribute.commandPalette(
        ...[
          ...
          id: `api-studio.palette.${name}`,
          title: `API Studio: ${name
            .split('-')
            .map((part) => part[0]?.toUpperCase() + part.slice(1))
            .join(' ')}`,
          commandId: `api-studio.${name}`,
          category: 'API Studio',
          order: 100 + index,
      ...
      context.contribute.menus(
        {
          id: 'api-studio.menu.new-request',
          title: 'New Request',
          location: 'sidebar/context',
          commandId: 'api-studio.new-request',
          group: 'api-studio.create',
        },
        {
          id: 'api-studio.menu.new-collection',
          title: 'New Collection',
          location: 'sidebar/context',
          commandId: 'api-studio.new-collection',
          group: 'api-studio.create',
        ...
        {
          id: 'api-studio.menu.duplicate',
          title: 'Duplicate Request',
          location: 'tab/context',
          commandId: 'api-studio.duplicate-request',
          contexts: { tabKinds: ['api-studio.request'] },
        },
        {
          id: 'api-studio.menu.log',
          title: 'Show Request Log',
          location: 'global/activity',
          commandId: 'api-studio.toggle-log',
          group: 'api-studio.views',
        },
      ),
    ]
