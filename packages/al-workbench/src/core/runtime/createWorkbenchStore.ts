...
export function createWorkbenchStore(
  ...
  function createUnavailableCollection<T extends WorkbenchTabSession | WorkbenchTabTemplate>(
    schema: T['schema'],
  ): WorkbenchTabCollectionService<T> {
    return {
      list: () => [],
      saveCurrent: async () => {
      ...
      importJson: async (json: string) => parseWorkbenchTabPayload<T>(json, schema),
      apply: () => undefined,
    }
  }
  ...
  const tabsService: WorkbenchTabWorkspaceServices = {
    ...
    sessions: createUnavailableCollection<WorkbenchTabSession>('activelane.workbench.tabs.session'),
    templates: createUnavailableCollection<WorkbenchTabTemplate>(
