// RingSpinner.ts
import { defineComponent, h } from 'vue'

export default defineComponent({
  name: 'RingSpinner',

  props: {},

  emits: [],

  setup(props, { emit }) {
    return () => {
      // <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><style>.spinner_VpEe{animation:spinner_vXu6 1.2s cubic-bezier(0.52,.6,.25,.99) infinite}.spinner_eahp{animation-delay:.4s}.spinner_f7Y2{animation-delay:.8s}@keyframes spinner_vXu6{0%{r:0;opacity:1}100%{r:11px;opacity:0}}</style><circle class="spinner_VpEe" cx="12" cy="12" r="0"/><circle class="spinner_VpEe spinner_eahp" cx="12" cy="12" r="0"/><circle class="spinner_VpEe spinner_f7Y2" cx="12" cy="12" r="0"/></svg>
      return h(
        'svg',
        {
          width: '24',
          height: '24',
          viewBox: '0 0 24 24',
          xmlns: 'http://www.w3.org/2000/svg',
        },
        [
          h(
            'style',
            {},
            `
.spinner_VpEe{animation:spinner_vXu6 1.2s cubic-bezier(0.52,.6,.25,.99) infinite}
.spinner_eahp{animation-delay:.4s}
.spinner_f7Y2{animation-delay:.8s}
@keyframes spinner_vXu6{
  0%{r:0;opacity:1}
  100%{r:11px;opacity:0}
}
        `,
          ),
          h('circle', {
            fill: 'currentColor',
            class: 'spinner_VpEe',
            cx: '12',
            cy: '12',
            r: '0',
          }),
          h('circle', {
            fill: 'currentColor',
            class: 'spinner_VpEe spinner_eahp',
            cx: '12',
            cy: '12',
            r: '0',
          }),
          h('circle', {
            fill: 'currentColor',
            class: 'spinner_VpEe spinner_f7Y2',
            cx: '12',
            cy: '12',
            r: '0',
          }),
        ],
      )
    }
  },
})
