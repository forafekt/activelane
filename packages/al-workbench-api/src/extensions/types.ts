...
import type { ServerExtensionDeclaration } from '../server'
...
/**
 * Compatibility shape for older manifests. New manifests should use the
 * server extension declaration fields from ServerExtensionDeclaration.
 */
export interface WorkbenchServerContribution extends Partial<ServerExtensionDeclaration> {
  entry?: string
  entrypoints?: WorkbenchExtensionServerEntrypoint[]
}
...
export interface WorkbenchExtensionServerEntrypoint {
  id: string
  kind: 'module' | 'process' | 'worker'
  command?: string
  args?: string[]
  cwd?: string
  module?: string
